...
// services/emailNotification.js - COMPLETE UPDATED VERSION
import nodemailer from 'nodemailer';
import dotenv from 'dotenv';
import Order from '../Models/Order.js';
...
export const sendPayerActionRequiredEmail = async (orderId, orderUrl = null, reminderType = null) => {
  try {
    ...
    if (reminderType === 'followup') {
      ...
      subjectPrefix = 'REMINDER - ACTION NEEDED:';
      messagePrefix = 'This is a reminder that ';
    }
    ...
    const customerResult = await transporter.sendMail(customerMailOptions);
    console.log(`Customer reminder email sent: ${customerResult.messageId}`);
    ...
    const adminResult = await transporter.sendMail(adminMailOptions);
    console.log(`Admin reminder email sent: ${adminResult.messageId}`);
...
const formatAdminOrderEmail = (order, previousStatus) => {
  ...
  let cancellationInfoHtml = '';
  if ((order.status === 'CANCELED' || order.status === 'VOIDED') && order.cancelReason) {
    cancellationInfoHtml = `
    <div style="padding: 15px; background-color: #f8f9fa; border-left: 4px solid #dc3545; border-radius: 6px; margin-bottom: 20px;">
      <h3 style="margin: 0 0 10px 0; color: #333; font-size: 16px;">CANCELLATION DETAILS</h3>
      <p style="margin: 5px 0;"><strong>Reason:</strong> ${order.cancelReason}</p>
      <p style="margin: 5px 0;"><strong>Cancelled At:</strong> ${order.cancelledAt ? new Date(order.cancelledAt).toLocaleString() : new Date().toLocaleString()}</p>
    </div>`;
  }
...
Cancelled At: ${order.cancelledAt ? new Date(order.cancelledAt).toLocaleString() : new Date().toLocaleString()}`;
  }
...
const getStatusInfo = (status, orderUrl = null, reminderType = null) => {
  ...
  // Return default status info for other statuses
  switch(status) {
    case 'COMPLETED':
      return {
        title: 'Order Confirmation',
    ...
    case 'CANCELED':
      return {
        ...
        messageColor: '#721c24',
        message: 'Your order has been cancelled due to incomplete payment process.',
        thankYouMessage: 'Thank you for your interest in VARONA. You can place a new order at any time.',
        additionalContent: `
        <div style="margin-top: 20px; padding: 15px; background-color: #f8f9fa; border-left: 4px solid #dc3545; border-radius: 4px;">
          <p style="margin: 0 0 15px 0; font-size: 15px;">This order has been cancelled because the payment process was not completed within the required timeframe. Your cart items and measurements have been saved.</p>
          <p style="margin: 0 0 15px 0; font-size: 15px;">You can start a new order at any time:</p>
          <div style="margin-top: 20px; text-align: center;">
            <a href="${process.env.NODE_ENV === 'production' ? process.env.FRONTEND_URL_PROD : process.env.FRONTEND_URL_LOCAL}/shop" 
              style="background-color: #0056b3; color: white; padding: 12px 20px; text-decoration: none; border-radius: 4px; font-weight: bold; display: inline-block;">Shop Now</a>
          </div>
        </div>`
      };
    ...
    case 'VOIDED':
      return {
        ...
        additionalContent: `
        <div style="margin-top: 20px; padding: 15px; background-color: #f8f9fa; border-left: 4px solid #dc3545; border-radius: 4px;">
          <p style="margin: 0 0 15px 0; font-size: 15px;">This order has been voided. If this was not expected, please contact our customer support.</p>
          <div style="margin-top: 20px; text-align: center;">
            <a href="${process.env.NODE_ENV === 'production' ? process.env.FRONTEND_URL_PROD : process.env.FRONTEND_URL_LOCAL}/shop" 
              style="background-color: #0056b3; color: white; padding: 12px 20px; text-decoration: none; border-radius: 4px; font-weight: bold; display: inline-block;">Shop Now</a>
          </div>
