...
//   const [error, setError] = useState(null)
//   const [paymentCancelled, setPaymentCancelled] = useState(false)
//   const [paymentComponentReady, setPaymentComponentReady] = useState(false)
...
//           // If order requires payment, prepare payment component
//           if (data && data.status === 'PAYER_ACTION_REQUIRED') {
//             // Small delay to prepare payment component smoothly
//             setTimeout(() => {
//               setPaymentComponentReady(true)
//             }, 500)
//           }
//         }
//       } catch (err) {
...
//     setPaymentComponentReady(false)
//     setTimeout(() => {
//       window.location.reload()
//     }, 500)
//   }
...
//   // Order requires payment - with controlled PayPal component rendering
//   if (order.status === 'PAYER_ACTION_REQUIRED') {
//     const formattedCartItems = order.items.map((item) => ({
//       id: item.productId || item.id,
//       name: item.name,
...
//           <div className={styles.paymentSection}>
//             <h3>Payment Options</h3>
//             {/* Only render PayPal component when ready to prevent flickering */}
//             {paymentComponentReady ? (
//               <PayPalPayment
//                 cart={formattedCartItems}
//                 measurements={measurements}
//                 deliveryDetails={deliveryDetails}
...
const OrderStatus = () => {
  ...
  // Simple state for PayPal component
  const [showPayPal, setShowPayPal] = useState(false)
  
  const paymentSetupRef = useRef(false)
    ...
    return (
      <div className={styles.container}>
        <div className={styles.paymentContainer}>
          ...
          <div className={styles.paymentSection}>
            <h3>Payment Options</h3>
            {/* Simple conditional rendering - show PayPal when ready */}
            {showPayPal ? (
              <PayPalPayment
                cart={formattedCartItems}
                measurements={measurements}
